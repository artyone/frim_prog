...
class Config():
    ...
    def create_config(self):
        with open(self.path_to_config, 'w', encoding='utf8') as file:
            config = {}
            if os.path.isfile(r'C:/Tikhonov/frim_prog/UO/atbackend/atprogram.exe'):
                config['app_path'] = r'C:/Tikhonov/frim_prog/UO/atbackend/atprogram.exe'
            else:
                config['app_path'] = ''
            config['fuse'] = 'E419FF'
    ...
    def create_dll(self):
        with open(self.path_to_dll, 'w', encoding='utf8') as file:
            dll = {}
            dll['disk_file'] = ['236B 4881 2B7D 2376 5690 90B8 BD23 1DC5',
                                '1111 4881 2B7D 2376 5690 90B8 BD23 1DC5']
            dll['device_file'] = ['236B 4881 2B7D 2376 5690 90B8 BD23 1DC5',
                                  '1111 4881 2B7D 2376 5690 90B8 BD23 1DC5']
    ...
    def get_command_hex(self):
        ...
        hex_filepath.replace('{time}', str(int(time.time())))
...
class Commander(object):
    ...
    def exec_program_commands(self):
    ...
    def exec_read_commands(self):
        ...
        if 'Firmware check OK' in answer:
            ...
            hash = self.get_md5(f'{config.config_data["hex_filepath"]}/{file_name}')
            return True, 'Firmware check OK', hash
        else:
            return False, answer, None

    @staticmethod
    def save_log(command, answer):
...
class Text_log(ScrolledText):
    ...
    def set_text(self, text):
        if not text or (text not in self.codes_err and text not in self.codes_suc):
            text = 'Unknown Error'
        if text in self.codes_err:
            rus_text = self.codes_err[text]
            color = 'red'
        else:
            rus_text = self.codes_suc[text]
            color = 'green'
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        s.theme_use('vista')
        self.title(
            "Программирование микроконтроллера Atmega / 1887ВЕ7Т, вер. 1.3")
        frame = tk.Frame(
            self,
            padx=10,
            pady=10
        ...
        self.txt_log = Text_log(frame, height=15, width=53)
    ...
    def send_frw(self):
        ...
        if config.is_rigth_app_path() and os.path.isfile(frm_filepath):
            ...
            self.txt_log.set_text(answer1)
            self.txt_log.set_text(answer2)
        else:
            self.txt_log('Wrong path')
    ...
    def read_frw(self):
        if config.is_rigth_app_path():
            ...
            self.txt_log.set_text('Wrong path')
    ...
    def __grid_interface(self):
        ...
        self.lbl_image.grid(row=11, column=3, padx=4, sticky='s')
        self.txt_log.grid(row=11, column=2, pady=5, padx=4)
        self.geometry(self.center_window(width=710, height=480))
